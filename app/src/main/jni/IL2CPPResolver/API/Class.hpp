...
namespace IL2CPP
{
    namespace Class
    {
        ...
        namespace Utils
        {
            ...
            template<typename T>
            void SetFieldValue(void* instance, const std::string& className, const std::string& fieldName, const T& value) {
                if(!instance) {
                    LOGE("Null instance, field %s  not found in class %s, ", fieldName.c_str(), className.c_str());
                    return;
                }
                int fieldOffset = GetFieldOffset(className.c_str(), fieldName.c_str());
                if (fieldOffset < 0) {
                    LOGE("Field %s  not found in class %s",fieldName.c_str(),className.c_str());
                    return;
                }
